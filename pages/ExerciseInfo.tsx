...
export default function ExerciseInfo() {
  ...
  useEffect(() => {
    supabase.auth
      .getSession()
      .then(({ data }) => setSignedIn(!!data.session?.user));
    (async () => {
      try {
        const resp = await fetch(
          `${SUPABASE_URL}/functions/v1/exercise-info?id=${encodeURIComponent(id || "")}`,
        );
      ...
      <div className="min-h-screen grid place-items-center p-6 text-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Check the QR code or ask a coach.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="p-4 flex items-center gap-2 border-b border-border">
        ...
        <span className="font-heading tracking-wider uppercase text-lg">
          Eat Train Live
        </span>
      </div>
      ...
      <div className="max-w-md mx-auto p-4 space-y-4">
        <div>
          <h1 className="text-2xl font-heading tracking-wide uppercase">
            {ex.name}
          </h1>
          <div className="flex gap-2 mt-1.5 flex-wrap">
            {[ex.equipment, ex.muscle, ex.difficulty]
              .filter(Boolean)
              .map((t: string) => (
                <span
                  key={t}
                  className="bg-primary/10 text-primary rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider"
                >
                  {t}
                </span>
              ))}
        ...
        <p className="text-sm text-muted-foreground">
          Scan any machine in the gym to see how to use it.
        </p>
        ...
        {signedIn ? (
          ...
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 text-center space-y-2">
            <p className="font-bold">
              Want your own plan, videos &amp; tracking?
            </p>
            <p className="text-sm text-muted-foreground">
              Join the Eat Train Live app — free to set up.
            </p>
            <button
            ...
            <p className="text-[11px] text-muted-foreground">
              On iPhone: tap Share → Add to Home Screen to install.
            </p>
          </div>
        )}
      </div>
