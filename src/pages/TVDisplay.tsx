    ...
    >
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <div className="flex items-center gap-2">
            <div className="text-right">
              <div className="font-heading text-2xl tracking-tighter leading-none">
                ETL
              </div>
          ...
          <Button
            variant="outline"
            size="sm"
            className="gap-1.5 bg-black/50 border-white/20 hover:bg-black/80 h-7 text-xs px-2"
      ...
      <div className="flex-1 w-full grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-4 items-start pb-4">
        {sections.map((section, sIdx) => (
          <div
            key={sIdx}
            className="rounded-lg overflow-hidden shadow-2xl flex flex-col border border-primary/20 bg-[#1a1a1a]"
          >
            {/* Section Header */}
            ...
            <div className="p-3 md:p-4">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr>
                    <th className="pb-2 text-[10px] md:text-xs font-bold text-muted-foreground uppercase tracking-widest w-[50%]">
                      Exercise
                    </th>
                    <th className="pb-2 text-[10px] md:text-xs font-bold text-muted-foreground uppercase tracking-widest text-center w-[15%]">
                      Sets
                    </th>
                    <th className="pb-2 text-[10px] md:text-xs font-bold text-muted-foreground uppercase tracking-widest text-center w-[20%]">
                      Reps
                    </th>
                    <th className="pb-2 text-[10px] md:text-xs font-bold text-muted-foreground uppercase tracking-widest text-center w-[15%]">
                      Rest
                ...
                <tbody className="space-y-1">
                  {section.exercises.map((ex, idx) => {
                    ...
                    const isLinkedToPrev =
                      idx > 0 && section.exercises[idx - 1].linkedToNext;
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-1.5 md:py-2 px-2 text-center font-bold text-sm md:text-base tabular-nums">
                          {ex.sets > 0 ? ex.sets : "-"}
                        </td>
                        <td className="py-1.5 md:py-2 px-2 text-center font-bold text-sm md:text-base tabular-nums">
                          {displayReps || "-"}
                        </td>
                        <td className="py-1.5 md:py-2 pl-2 text-center font-bold text-sm md:text-base tabular-nums">
                          {ex.linkedToNext ? "—" : ex.rest > 0 ? ex.rest : "0"}
                        </td>
                      </tr>
                    );
                  })}
