...
export default function MachinePage() {
  ...
  useEffect(() => {
    ...
    fetch(
      `${SUPABASE_URL}/functions/v1/exercise-info?ids=${encodeURIComponent(ids.join(","))}`,
    )
      .then((r) => r.json())
      .then((data) => {
  ...
  const variations = (machine.exercises || [])
    .map((v: any) => ({
      v,
      src:
        v.exerciseId && videos[v.exerciseId]
          ? embedSrc(videos[v.exerciseId].videoUrl)
          : null,
    }))
    .filter((x: any) => x.src);
      ...
      <div className="max-w-md mx-auto p-4 space-y-5">
        ...
        {!signedIn && (
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 text-center space-y-2">
            <p className="font-bold">
              Get the full app — plans, videos &amp; tracking
            </p>
            <button
        ...
        {variations.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-4">
            Videos coming soon for this machine.
          </p>
        )}
