...
interface CoachPastWorkoutsProps {
  staffSecret: string;
  memberUserId: string;
  exercises: any[];
}
...
export function CoachPastWorkouts({
  staffSecret,
  memberUserId,
  exercises,
}: CoachPastWorkoutsProps) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const ws = await listMemberWorkouts(staffSecret, memberUserId);
      setWorkouts(ws);
    } catch (e: any) {
      console.error("listMemberWorkouts failed", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    if (!draft) return;
    setSaving(true);
    const res = await updateMemberWorkout(staffSecret, draft.id, draft);
    setSaving(false);
    if (res.success) {
      ...
      toast.error("Couldn't save — " + (res.error?.message || "try again"));
    }
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    setDeleting(true);
    const res = await deleteMemberWorkout(staffSecret, deleteId);
    setDeleting(false);
    if (res.success) {
      ...
      toast.error("Couldn't delete — " + (res.error?.message || "try again"));
    }
  };
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently removes the session from the member's history.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
